...
import { SubmitExpenseDialog } from "@/components/SubmitExpenseDialog"; // <-- IMPORT THE DIALOG

const DashboardLayout = () => {
  const navigate = useNavigate();
  const { user, logout } = useAuth();
  const [isExpenseDialogOpen, setIsExpenseDialogOpen] = useState(false); // <-- STATE TO CONTROL DIALOG
        ...
        <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
          <div className="container flex h-14 max-w-screen-2xl items-center">
            ...
            <div className="ml-auto flex items-center gap-4">
              ...
              <Button onClick={() => setIsExpenseDialogOpen(true)}>Submit Expense</Button>
              <ThemeToggle />
              <DropdownMenu>
